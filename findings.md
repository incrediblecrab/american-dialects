...
**What it looks like on one person.** The aggregate result is 253 matched comparisons, which is convincing and impossible to picture. One worked example, exported by `model/export_web.py` and reproduced live on the published site, makes the mechanism visible. A speaker gives the most common Pittsburgh answer to each of the fourteen questions the quiz asks — Pittsburgh because it has the sharpest lexical signature in the survey, so any error anywhere in the chain shows up. The same fourteen answers, scored twice:

| | 80% region | modal cell | strongest metro |
|---|---|---|---|
| ρ = 0.177 | 1,752,029 km² | New York | New Castle, PA at 0.6% |
| ρ = 0 | **786,764 km²** | Ohio–Pennsylvania border | New Castle, PA at 2.2% |

The discounted region is 965,266 km² larger and its modal cell has crossed two states. The more telling number is the 0.6%: after fourteen answers that all point at one metropolitan area, the discounted model's strongest single place is barely a preference. It has not become uncertain in a useful way, it has slid back onto the population prior, which is a statement about where Americans live rather than about how this person talks.

Sweeping the quiz length makes the point sharper still. Under the discount the 80% region stops contracting after the fourth question and sits between 1.68 and 1.83 million km² at every length from four to twenty — it is not integrating evidence at all past that point. Under ρ = 0 it keeps contracting throughout. One speaker is an anecdote and this one was chosen to be legible, so it is offered as an illustration of the mechanism rather than as evidence for it; the evidence is the 253 comparisons.
...
## Does the network recognise a mover? No, and in this simulator it cannot

Part of the case for building `model/neural.py` was that a factorised likelihood multiplies independent evidence and therefore concentrates as answers arrive, while a network sees the whole answer vector at once and could in principle learn to recognise a mover and widen appropriately rather than concentrating. That was an argument, not a result. It has now been measured. **The network does not do it, it degrades on movers slightly worse than Bayes does rather than better, and the reason is that under the process it is trained on there is nothing there to find.**

First the shape of the thing, because it decides whether the question is even askable. The network's output is a softmax over 1024 population-weighted centroids, splatted back over all 50,888 cells in proportion to the prior, so it emits a genuine distribution and widening is a behaviour it is capable of having. That was checked before the experiment was designed; had the output been a point estimate the question would have been settled without measuring anything.

**The measurement.** `model/mover.py diagnose` draws 6,000 held-out people from the same process the network was trained against, and scores every model on identical people at identical question counts. 863 of them moved, with a median displacement of 1,499 km, so these are not people who crossed a county line. Error is measured against `home`; the answers were generated from `speech`. Alongside the network and the deployed Bayesian model there are three controls: the same Bayes posterior pushed through the network's 1024-class bottleneck and splatted back, which prices the representation rather than the learning; the retired discount, which is the scalar temper the original argument was set against; and the population prior itself, which is what a model that ignores the answers entirely would say.

**It does not widen.** The quantity is the ratio of the median 80% credible area on movers to the same area on non-movers, because a model that is simply always vaguer has done nothing clever. At one, five, twelve, fourteen, twenty and thirty questions the network's ratio is 1.00, 1.02, 1.02, 1.01, 0.93 and 0.99, and every bootstrap interval contains one. Bayes at zero discount gives 1.00, 1.01, 1.02, 1.01, 0.98 and 1.04, which is the same answer. A sharper form of the question is whether a mover can be picked out from the width alone: the AUC of the 80% area as a mover detector runs between 0.493 and 0.516 for every model at every length, and every interval contains 0.5. On 863 movers and 5,137 non-movers, the network's posterior is exactly as wide for somebody raised 2,000 km from where they now speak as for somebody who never left.

**It degrades more, not less.** At the fourteen questions the quiz asks, the network places non-movers at a median of 209 km and movers at 1,503 km; Bayes places them at 258 km and 1,455 km. The network is 49 km better on the people it was already better on and 48 km worse on the ones the argument was about. Its ratio of mover error to non-mover error is 7.2 against Bayes's 5.6, so on the very measure the claim was made in, the network is the more brittle of the two. **Both are worse than not asking.** A model that ignores every answer and reports the population prior places these same movers at 1,336 km. From five questions onward both models sit further from a mover's home than a model that never looked at their answers, because the answers are real evidence about a real place and that place is the wrong one.

**The 80% region is not an 80% region for anybody.** Split by mover status at fourteen questions, the network's nominal 80% region contains the true home 93.3% of the time for non-movers and 32.9% of the time for movers; Bayes reads 92.8% and 36.5%. Pooled, they are 84.6% and 84.7% — which is the coverage this report quotes elsewhere as within a few points of nominal. That pooled number is the average of a model that is far too timid and a model that is confidently wrong, and it describes neither. The split widens with every question asked: mover coverage for the network is 71.7% at one question, 52.6% at five, 32.9% at fourteen and 20.0% at thirty. Asking more questions makes the stated confidence more honest for the 85% of people who did not move and less honest, monotonically, for the 15% who did. An independent replication at a different seed, also 6,000 people and 896 movers, gives 93.3 / 29.0 / 83.7 for the network and 93.1 / 33.9 / 84.3 for Bayes, so this is not one unlucky draw.

**Why nothing widened.** `model/idiolect.py` draws a mover's speech cell from `g.prior`, which is the same distribution `home` is drawn from, and draws the idiolect independently of mover status. The probability of any answer vector is therefore the same whether the person moved or not, so the probability of having moved given the answers is exactly the mover rate for every possible answer vector. The exact posterior over home follows in a line: writing Q(c) for the prior times the likelihood and Z for its sum, the posterior is (1 - m) times the ordinary posterior plus m times the prior. **The correct response to movers in this world is to widen every posterior by the same fixed admixture of prior.** Selective widening is not merely hard here; it is wrong, because there is nothing to select on. A mover's answers describe a real place, perfectly consistently, and somebody who genuinely comes from that place produces the same answers with the same probability.

**That is a derivation, so it was also measured.** `model/mover.py probe` trains a classifier on the identical 802-bit encoding the geolocation network is handed and asks it to predict mover status. On 300,000 people, 60,000 held out, the AUC is 0.5002 at five questions, 0.4993 at fourteen and 0.5039 at thirty, with intervals of about six thousandths that all contain 0.5. Trained on the same labels randomly permuted — pure noise — it reads 0.5001, 0.4984 and 0.5003, which is the same thing. The positive control is what makes that a null rather than a failure to train: the same architecture, optimiser and inputs, asked instead for the sign of the latent idiolect — a variable that genuinely is written into the answers — reaches 0.760, 0.850 and 0.872. A model-free version agrees: comparing the choice distribution of movers and non-movers question by question, 1 of the 30 deployed questions is significant at the five percent level against 1.5 expected by chance, with a median p-value of 0.587.

**The one thing the network does do better, and it is not the claimed thing.** Movers are far better served by the network in the extreme tail. At thirty questions the network's nominal 95% region contains a mover's home 76% of the time against Bayes's 38%. That is a large, real difference and it is not the 1024-cluster quantisation — the control that pushes the Bayes posterior through the same bottleneck reads 39%. The network genuinely learned to hold more mass far from its mode than a factorised likelihood does. But it is not selective. Its 95% region measures 1.852 million km² on non-movers and 1.842 million on movers, a ratio of 1.00. The network is uniformly heavier-tailed for everybody, and movers benefit only because a mover's home is exactly where a heavy tail lands. That is a duller mechanism than recognising anybody, and it is available to the Bayesian model for nothing: the mixture derived above, applied to the deployed posterior at weight 0.15, takes the same number to 78%.

**Why the mixture is not being deployed anyway.** `model/mover.py mix` sweeps the admixture weight and scores it properly, as a log score at the true home cell on the same held-out people. At the deployed fourteen questions the mixture at the true mover rate is worth +0.0137 nats, interval -0.0007 to +0.0291 — within noise. It is a clear loss at shorter quizzes, -0.0443 nats at five questions, and a clear gain only past twenty. The reason is that `TAU_BASE` is already doing part of the job. The weight that actually maximises the score climbs from 0.000 at five questions through 0.050 at fourteen to 0.090 at thirty, approaching the true mover rate without reaching it, because a model already tempered is already too wide to want the whole correction. For the network the best weight is 0.015 at every length past one. **At the length this quiz actually asks, an explicit mover correction buys nothing measurable in either model, and it should not be added on the strength of a result that only appears past twenty questions.**

**The auxiliary mover head was not built, and this is the reason.** The plan was a second output predicting whether the person moved, trained jointly, on the theory that a shared trunk would learn features useful to both. A head can only help if its target is predictable from its input, and the target here is not weakly predictable or hard to predict but independent of the input by construction, with a proof and a null at 0.4993 to go with it. Training it would produce a head that outputs the base rate for everybody and a gradient that is pure noise into the trunk. It was skipped for that reason and not for cost.

**What this does and does not say about people.** All of the above is a statement about a simulator, and the assumption doing the work is that a mover's speech comes from a cell drawn independently from the population prior. That produces somebody linguistically indistinguishable from a native of wherever they were raised, and such a person genuinely cannot be detected from a dialect questionnaire — the model is not failing, it is being asked a question with no answer. Real movers are probably not like that. They blend: an adult who spent twenty years in Alabama and fifteen in Oregon plausibly gives an answer vector that is internally inconsistent, matching Alabama on the vocabulary acquired young and Oregon on the items that shift later, and that inconsistency is a signature a non-factorised model really could learn to read. **The original claim may well be true of real movers. It is not true of the movers this project simulates, and the network has therefore never had the opportunity to learn the thing it was partly built to do.** Fixing that means drawing a mover's answers from a mixture of two cells rather than from one wrong cell, which is a larger change than it sounds, because the blend fraction would have to vary by question and be calibrated against something, and no dataset in this project can calibrate it. Until that exists, the second of the two stated justifications for the network should be treated as withdrawn. The first — that it never factorises, and so is not forced into an independence assumption the data violates — is untouched and is still supported by the accuracy table above.

## Geography and idiolect, question by question

The design effect was measured pooled: one number for all 122 questions. Per question it obviously is not uniform. Some questions ask about a word whose boundary you could draw on a map. Others ask how *you* say a word, and the answer varies between people who grew up on the same street. A question of the second kind costs the respondent a tap and returns almost nothing about where they are from, so it should be possible to rank questions by how much of their variance is geography and how much is idiolect, and to build a better quiz out of the top of that ranking.

Measuring the split is straightforward and the result is clean. Each question yields two mutual informations on the same scale: the bits its answer carries about the grid cell, and the bits it carries about the latent personal axis, both computed exactly by quadrature under the tilt model the simulator already uses. Their ratio runs from 0.15 to 7.27 with a median of 1.02, and `model/signal_split.py` writes it to `data/model/question_signal.csv`.

The ranking is what a dialectologist would write down unprompted. At the top: what "the City" means, TP-ing a house, the grey creature that rolls into a ball, the sweetened carbonated beverage, the night before Halloween. At the bottom: the "s" in chromosome, the -sp- in thespian, amphitheater, citizen, anniversary. Split by class, the 31 pronunciation prompts have a median ratio of 0.47 against 1.29 for the 91 lexical and syntactic questions. The check worth making is the exception rather than the rule, because an estimator that merely rediscovered "phonetics is noisy" would put every phonetic item at the bottom. It does not. Cot/caught ranks 14th of 122, Mary/merry/marry 20th and aunt 21st — precisely the three pronunciation variables in the Harvard set that are genuine American isoglosses. The estimator separates them from the rest of the phonetics without being told they are different.

Then the hypothesis fails. Ranking questions by the ratio instead of by mutual information produces a worse quiz. On 8,000 identical simulated people, paired, the ratio ordering is worse at 21 of the 28 quiz lengths where it picks a different set and better at one, costing a mean of 44 km and 64 km [46, 82] at the deployed fourteen. A blended criterion that penalises idiolect bits without discarding information costs 19 km. The only variant that even draws is one that ranks on the geographic share of an answer's total information, which comes out 4 km ahead on average, significantly better at three lengths and significantly worse at two — within noise, and not worth a second selection criterion to obtain. All of it replicates on an independent draw of 8,000 people. The reason is visible in the ranking itself: mutual information and the ratio correlate at 0.945, so the ratio is not adding a new axis, it is tilting an existing one. What tilting does is trade away information. The questions the ratio promotes into the first fourteen are cleaner per bit and carry fewer bits, and at fourteen questions the model is still bit-starved, so the trade loses.

This is a clean negative result and it settles the question: mutual information already captures what matters for selection. The extra machinery is measured, it is correct, it is documented, and it is not deployed.

The obvious next move after measuring a per-question design effect is to use it to discount each question's evidence individually. That is a per-question version of exactly what the pooled discount did, and it fails the same way, so it was tested once as a labelled arm and left there. At fourteen questions it gives 818 km against 351. It is best at thirteen questions and then gets 124 km worse by thirty, which is the same turnover at the same place as the pooled discount it generalises. It does not merely resemble the pooled failure; it reproduces it, and the reason is measurable. The mean pairwise idiolect correlation over the deployed question set is 0.159 to 0.181 depending on length, against the pooled figure over all 122. The questions the quiz asks are not unusually clean, so a per-question discount has almost nothing to be per-question about, and collapses to the pooled discount that was already the largest single source of error.

Two caveats stated plainly. The estimator and the simulator share the tilt model, so this is a model-based decomposition anchored on the measured pooled correlation, not an independent measurement of idiolect — the split between questions is real, but its absolute scale inherits whatever the pooled estimate got wrong. And the one external check available runs mildly against the ratio: over the 25 questions comparable with the Cambridge survey, mutual information correlates with cross-survey agreement at Spearman 0.53 while the ratio correlates at 0.28. If anything, the outside data says mutual information is the better guide to which questions replicate, which is what the head-to-head found by a different route.

## What a boundary actually looks like

An isogloss is the line where one word gives way to another, and drawing one takes two words rather than one. A single answer's map is a heat map, and mostly a picture of where people live; subtracting one recovered surface from another gives the odds of the first word against the second at every point, and the contour where those odds are even is the boundary. That is what the published explorer draws, for eight contrasts.

Drawing them that way makes something measurable that an atlas hides. If the boundary is the contour where the odds are even, then the *width* of the boundary is the ground over which the odds swing from three-to-one one way to three-to-one the other — band area divided by contour length, a mean thickness in kilometres. Measured on the quantised surfaces the browser actually downloads, rather than on the float originals, so the number describes the artefact on the screen.

The eight range from **73 km** for *yinz* against *y'all* in western Pennsylvania to **443 km** for *catty-corner* against *kitty-corner*. The first is narrower than the drive across the county. The last is not really a boundary at all; it is a national preference with a slight tilt, and calling it an isogloss is a decision about drawing rather than a fact about speech. Printed as a line, in the way every atlas prints them, the two are indistinguishable — and that is the point the section is built to make.

Two honest limits. The widths are properties of the *fitted* surfaces, which are Gaussian-smoothed at a cross-validated bandwidth, so smoothing can only widen a boundary and never sharpen one: each width is an **upper bound** on how abrupt the real thing is. And the recovery is validated at state level, so a line drawn *inside* Pennsylvania is exactly the sub-state claim this project cannot yet check against ground truth. The explorer says so where it draws them.

One contrast was cut rather than shipped. *Grinder* against *sub* is a well-known New England boundary, but Boston reads only 17% *grinder* in the recovered surfaces, meaning the marked word loses at its own anchor city, and a boundary drawn around a word that is not locally dominant is a picture of nothing. `model/export_web.py` now refuses to export any contrast in that condition rather than leaving it to be noticed by eye. For the same reason *yinz* is paired against *y'all* and not against *you guys*: against the national default the yinz region collapses to a fraction of a percent of land and Pittsburgh itself reads under a quarter, which is a true statement about relative frequency and a misleading picture of where the word lives.
...
| questions | discriminative net | Bayes, ρ = 0 (deployed) | Bayes, ρ = 0.177 (the old discount) |
|---|---|---|---|
| 1 | 1193 | 1123 | 1123 |
| 3 | 871 | 942 | 1058 |
| 5 | 654 | 685 | 897 |
...
This table replaces one that stopped at twenty questions and was computed on an ordering the discounted model had chosen for itself. Both defects are now fixed: the ordering is re-derived with the discount off, and the sweep runs to thirty. The k = 20 column is unchanged to the kilometre — 199, 264 — because the first twenty questions are the same *set* in both orderings, only permuted. That the two independent runs agree exactly there is the best available evidence that nothing else moved.

**Twelve is not a fact about dialects. It is an artefact of the discount.** On a clean ordering the discounted model bottoms out at thirteen questions and then climbs 109 km by thirty, at −7.6 km per question over 13–20 and −5.6 km per question over 20–30. It stops because the temper has finally outrun the evidence. Both correctly specified models descend monotonically to the cap with no turnaround anywhere: the net gains 25.7 km per question over 13–17 and is still gaining 2.7 km per question at thirty, and ρ = 0 Bayes 3.4 km per question at thirty.

Note that the discounted model's exact turning point is a property of the ordering, not of the discount. On the ordering it chose for itself it bottomed at eleven-to-twelve and shed 88 km by twenty; on the clean ordering it bottoms at thirteen and sheds 109 km by thirty. The number moves, the phenomenon does not, and the previous report's 88 km should be read as one instance of it rather than as the quantity.

**Model choice dominates question count, by a wide margin.** At the same twelve questions the network cuts the discounted error by 61%, 891 km to 347. The sharper comparison is that the network with **five** questions (654 km) beats the discounted model with *any* number of questions, whose best over thirty is 794 km. No quiz length rescues a model that has stopped learning, and no amount of extra asking substitutes for removing the discount.

**Where the returns actually stop — and the honest answer is that they have not.** Neither correct model has turned by thirty questions, so there is no interior optimum to report and any stopping point is a stated rule rather than a measurement. Two rules, applied to the reduction available between one and thirty questions: 50% of it arrives for the network by five questions, 75% by ten, 90% by sixteen and 95% by twenty; for ρ = 0 Bayes, 50% by seven, 75% by thirteen, 90% by nineteen and 95% by twenty-three. Priced explicitly — how many km a question must save to be worth asking — the network's optimum is eighteen questions at 10 km, fifteen at 20 km, eleven at 30 km and seven at 50 km; the deployed Bayesian model's is twenty-four, fifteen, **fourteen** and five.

An earlier version of this section reported that the 90% rule and the 30 km price both landed on fourteen for the network, and treated the coincidence as support. **The coincidence did not survive extending the sweep.** At thirty questions the two rules give sixteen and eleven for the network, and nineteen and fourteen for Bayes. They bracket rather than agree. Fourteen is retained because it is the priced optimum of the model actually deployed and falls inside the network's bracket, not because two criteria pick it out.

**Four caveats, none of them small.** First, this population has **no surface error**, so these numbers are not comparable to the headline table above, which carried 20 points of it; that is why ρ = 0 Bayes reads 264 km here and 329 km there at twenty questions. The network has therefore never been trained against wrong surfaces, which is the exact failure `TAU_BASE` exists to absorb — this is the most important remaining gap in it. Second, the sweep now stops at thirty because `question_order.csv` has thirty rows, and both good models are still improving there, so the true optimum is still beyond what has been measured; the cap has moved, not gone. Third, the previous report suspected that the network's flattening over questions 17–20 was an artefact of a tail ranked by a contaminated search. **It was not.** On the clean ordering the network gains 25.7 km per question over 13–17 and 7.5 over 17–20, against 6.1 on the contaminated one — essentially the same flattening, from a tail chosen by a different search. The suspicion was reasonable and it was wrong; the network really does saturate there, and only resumes falling further out. Fourth, the network wins on distance at every length past two but **loses on modal state at twenty**, 46.5% against 48.0%, and the gap widens rather than closes with more questions — 52.4% against 54.6% at thirty. That is the 1024-cluster quantisation blurring state boundaries that Bayes resolves at full cell resolution. It is better at *where*, slightly worse at *which state*, and no amount of asking fixes it.

## The recommendation

**`RHO` is now 0.** This was a one-line change in `model/infer.py` and it was the largest single improvement available: three times the precision at twenty questions, from answers the model already had. It was not a close call — 253 of 253 matched comparisons, across correlations up to 0.30, with and without movers, at four levels of surface error. The measurement of ρ was sound; the design effect was the wrong tool to apply it with. **Made**, together with the question count below, because doing either alone would have made the model worse.
...
**`N_QUESTIONS` is now fourteen — and fourteen is a judgement, not an optimum.** With the discount off, error falls at essentially every question, in every world tested including the worst corner of the design. "Ask twelve" was never a fact about dialects, and the second model makes the mechanism unmistakable: the discounted configuration bottoms out at thirteen questions and then *gets worse*, climbing 109 km by thirty, because the schedule discounts itself faster than the questions can inform it. Twelve was the correct stopping point for a broken model and for nothing else.

For a correctly specified one there is no stopping point in the measured range at all: both models are still improving at thirty. So the number has to come from a rule about diminishing returns, and it should be read as one. Priced at a plausible boredom cost of 30 km per question, the deployed Bayesian model's optimum is fourteen; the 90% rule says nineteen. Fourteen is the priced optimum of the model that ships, and it also keeps a quiz short enough that a stranger finishes it. That second consideration is not a research finding and is not presented as one.

**Consider replacing the model, not just the constant.** `model/neural.py` reads the same surfaces without factorising the likelihood, and on stress-tested people it cuts the discounted error by 61% at equal quiz length — 347 km against 891 at twelve questions — and beats that model's *best-ever* accuracy using five questions instead of twelve. Against the model now deployed the margin is narrower but consistent: 291 km against 361 at the fourteen questions the quiz asks, and 148 against 176 at thirty. It is still not ready to ship: it has never been trained against wrong surfaces, and it trades a little modal-state accuracy for its distance advantage, a gap that widens rather than closes with more questions. Its question ordering is no longer inherited from a contaminated search, which removes one of the three original objections.

**Ship the surfaces as bytes.** The browser payload turns on whether the likelihood surfaces survive being quantised to one byte each, so that was measured rather than argued: `model/export_web.py --measure` replays 300 simulated games at the deployed quiz length against both the full-precision surfaces and the byte-quantised ones, and they name the same cell in 87.7% of them. The interesting part is the disagreements. They are not small errors scattered around the answer; they are ties. In the worst of them the cell the quantised model picked was already holding 0.9712 of the winner's probability under full precision, so the surfaces did not change the model's mind, they broke a near-dead heat differently. The largest displacement was 61.2 km, which is inside the resolution the model can honestly claim and an order of magnitude below the error being measured. Quantisation is therefore free at this scale, and the payload can be bytes. Note that this number is not recomputed by `check.py`, because it needs the untracked likelihood arrays; what is enforced is that the figures in this paragraph are the figures the shipped site was built from.

**You may now quote a confidence number, with two caveats.** Across every length and every misspecification constructed, the credible regions cover within four points of nominal, and usually within three. That is a real change from the previous position. The first caveat is that this is still a simulation: the prior and the surfaces are the model's own, deliberately damaged in three ways whose *magnitudes* are calibrated against measurements but whose *shapes* are assumptions. Say "eighty percent" and mean it; do not say "eighty-three".

The second caveat was found later, by splitting that same coverage figure by mover status, and it is the sharper of the two. **The number is honest about a population and not about a person.** At the deployed quiz length the nominal 80% region contains the true home 93.3% of the time for the 85% of people who did not move and 32.9% of the time for the 15% who did; pooled, that is the 84.6% quoted above. The aggregate is within four points of nominal because it averages a model that is far too timid over a model that is confidently wrong, and it describes neither group. It also gets worse in one direction with every question asked, since mover coverage falls monotonically from 71.7% at one question to 20.0% at thirty while non-mover coverage holds. Quote the eighty percent to a room; do not promise it to a reader who tells you they grew up somewhere else. The measurement is in [does the network recognise a mover](#does-the-network-recognise-a-mover-no-and-in-this-simulator-it-cannot).
