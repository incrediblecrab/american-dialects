...
`web/` is the public version, built with [Astro](https://astro.build) and deployed to GitHub Pages by `.github/workflows/pages.yml`. It is a static build with no server behind it: the model runs in the browser, and every figure in the prose is baked in at build time from `web/src/content/generated.json`.
...
npm run preflight                    # everything the deploy runs, before pushing
```

`preflight` exists because the one failure this build has actually had is a lockfile that installs here and not on the runner. `package-lock.json` is written on macOS and replayed on Linux by `npm ci`, and the packages that ship a compiled binary publish one tarball per platform: whether the Linux one is recorded depends on how the tree happened to resolve, so a lockfile can be correct on the machine that wrote it and still be missing what the runner needs. `npm run lint:lockfile` reads the lockfile and asserts that each of the per-platform families in it has a member a linux/x64 runner would accept, which is the part that cannot otherwise be checked without a Linux machine. The deploy runs it too, so the failure arrives named rather than as an `npm ci` error about a lockfile being out of sync.
...
The prose is written in MDX, so the essay is edited as Markdown rather than as JSX. Each chapter is a route: `web/src/pages/*.mdx`, sharing one shell in `web/src/layouts/Base.astro`. There was a documentation theme here at first, and it was the wrong frame for the work — a sidebar tree, a search field and a per-page contents rail are all machinery for a reader looking something up, not for one being walked through an argument, and wrapping a single essay in them presented a piece of research as an API manual while squeezing the map, which is the point, into a third of the width. `web/src/components/prose/` holds the wrappers the layout does not provide — the full-bleed figure, the claim ladder on the limits page, and `Stat`, which is how a number reaches the page. Interactive components are imported into the MDX and given a client directive there, which is why `recovery.mdx` can put a scrubber in the middle of a paragraph without becoming a component itself.

Three rules that fall out of this and are worth knowing before editing an `.mdx` file. Numbers still come only from `generated.json`; `check.py` scans `.mdx` alongside `.ts`, `.tsx` and `.astro` and fails on any generated figure typed into the prose. Markdown on its own line is block content, so a wrapper meant to hold prose has to be a `div` — a `p` wrapping Markdown's own `p` is invalid, and a browser closes the outer one early, which strips the styling from the text it was marking. And links between chapters are real page links, so they carry the `/american-dialects/` base rather than being in-page anchors.

The generated content sits at `web/src/content/index.ts` and `generated.json`. It is imported directly by the pages that quote it rather than being loaded as a content collection, because it is the source of truth for every number the site prints and it is not a page.

The page ships one colour scheme. It reproduces a printed artifact from 2003 — a white sheet with blue dots on it — and on a dark ground that sheet reads as a hole burned in the page rather than as a document. A dark scheme would also mean a second full palette, a second map ramp and a second diverging pair, each of which would have to be kept in step with the first, so light-only removes the defect and the duplicate together.

The maps are drawn in an Albers Equal Area Conic, the projection US thematic maps are printed in, with the USGS standard parallels of 29.5°N and 45.5°N. The cells are stored on a regular latitude/longitude grid, so the cheap thing would be to draw one cell per pixel — but that is a plate carrée, and it renders the parallels flat, which makes the country read as a ragged rectangle rather than as the fan shape an atlas prints. Equal area matters here for more than appearance: the headline figure is an area in square kilometres, so a projection that inflated the north would make the northern half of every posterior look more important than it is. `web/src/model/albers.ts` reprojects by walking the output raster and asking what lies underneath each pixel, which avoids the gaps that scattering cells forward would leave, and caches that answer so a redraw costs one table lookup per pixel and the map stays cheap enough to animate on every answer.
